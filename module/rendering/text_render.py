...
from hyphen.dictools import list_installed as HYPHENATOR_LANGUAGES
...
def rotate_image(image, angle, upscale_ratio=None):
    if angle == 0:
        return image, (0, 0)
    image_exp = np.zeros((round(
        image.shape[0] * 1.5), round(image.shape[1] * 1.5), image.shape[2]), dtype=np.uint8)
    diff_i = (image_exp.shape[0] - image.shape[0]) // 2
    diff_j = (image_exp.shape[1] - image.shape[1]) // 2
    image_exp[diff_i:diff_i+image.shape[0],
              diff_j:diff_j+image.shape[1]] = image
    ...
    result = cv2.warpAffine(
        image_exp, rot_mat, image_exp.shape[1::-1], flags=interpolation)
    if angle == 90:
        return result, (0, 0)
...
def add_color(bw_char_map, color, stroke_char_map, stroke_color):
    ...
    if stroke_color is None:
        stroke_color = color
    bg = np.zeros(
        (stroke_char_map.shape[0], stroke_char_map.shape[1], 4), dtype=np.uint8)
    bg[:, :, 0] = stroke_color[0]
    bg[:, :, 1] = stroke_color[1]
    bg[:, :, 2] = stroke_color[2]
    ...
    bg[y:y+h, x:x+w, :] = (fg_alpha[:, :, np.newaxis] * fg[:, :, :] +
                           bg_alpha[:, :, np.newaxis] * bg[y:y+h, x:x+w, :])
    # Hợp nhất alpha: alpha_out = max(alpha_fg, alpha_bg)
    bg[y:y+h, x:x+w, 3] = np.maximum(bg[y:y+h, x:x+w, 3], fg[:, :, 3])
...
def get_font_line_metrics(font_size: int, direction: int = 0) -> tuple[int, int, int]:
    """
    Trả về (line_height, ascender, descender) theo pixel cho font hiện tại ở kích thước chỉ định.

    - line_height: chiều cao dòng đề xuất (ưu tiên face.size.height, fallback asc+desc)
    - ascender: chiều cao phần trên baseline
    - descender: chiều sâu phần dưới baseline (giá trị dương)
    """
    global FONT_SELECTION
    if not FONT_SELECTION:
        # Nạp fallback fonts nếu chưa có
        set_font('')
    face = FONT_SELECTION[0]
    if direction == 0:
        face.set_pixel_sizes(0, font_size)
    else:
        face.set_pixel_sizes(font_size, 0)
    # FreeType trả về đơn vị 26.6 (1/64 px)
    height = face.size.height >> 6
    asc = face.size.ascender >> 6
    desc = -(face.size.descender >> 6)
    if height <= 0:
        height = asc + desc
    return int(height), int(asc), int(desc)
...
def get_char_border(cdpt: str, font_size: int, direction: int):
    ...
    for i, face in enumerate(FONT_SELECTION):
        ...
        if direction == 0:
            ...
            face.set_pixel_sizes(font_size, 0)
        face.load_char(cdpt, freetype.FT_LOAD_DEFAULT |
...
def get_pair_kerning(prev_cdpt: str, curr_cdpt: str, font_size: int, direction: int = 0) -> int:
    """
    Trả về kerning theo pixel giữa hai ký tự ở kích thước font nhất định.
    Giá trị dương/âm sẽ dịch khoảng cách ngang trước khi vẽ ký tự thứ hai.
    """
    global FONT_SELECTION
    if not FONT_SELECTION:
        set_font('')
    ...
    if direction == 0:
        ...
        face.set_pixel_sizes(font_size, 0)
    try:
        if hasattr(face, 'has_kerning') and face.has_kerning:
            left = face.get_char_index(prev_cdpt)
            right = face.get_char_index(curr_cdpt)
            if left != 0 and right != 0:
                vec = face.get_kerning(
...
def calc_vertical(font_size: int, text: str, max_height: int):
    ...
    for i, cdpt in enumerate(text):
        ...
        line_width_right = max(
            line_width_right, char_width - abs(char_bearing_x))
    # last char
    line_text_list.append(line_str)
    line_height_list.append(line_height)
    # line_width_list.append(line_width_left + line_width_right)
...
def put_char_vertical(font_size: int, cdpt: str, pen_l: Tuple[int, int], canvas_text: np.ndarray, canvas_border: np.ndarray, border_size: int):
    ...
    # 这是原始字符的 bitmap 对象 / This is the bitmap object of the original character
    bitmap = slot.bitmap
    ...
    bitmap_char = np.array(bitmap.buffer, dtype=np.uint8).reshape(
        (char_bitmap_rows, char_bitmap_width))
    ...
    if paste_y_start >= paste_y_end or paste_x_start >= paste_x_end:
        logger.warning(
            f"Char '{cdpt}' is completely outside the canvas or on the boundary, skipped. Position: x={char_place_x}, y={char_place_y}, Canvas size: {canvas_text.shape}")
    else:
        # 确保切片源和目标尺寸匹配
    ...
    if border_size > 0:
        ...
        # 基于字体大小的比例值 / Proportional value based on font size
        ...
        if border_bitmap_rows * border_bitmap_width > 0 and len(bitmap_b.buffer) == border_bitmap_rows * border_bitmap_width:
            ...
            bitmap_border = np.array(bitmap_b.buffer, dtype=np.uint8).reshape(
                (border_bitmap_rows, border_bitmap_width))
            ...
            paste_border_y_end = min(
                canvas_border.shape[0], pen_border[1] + border_bitmap_rows)
            paste_border_x_end = min(
                canvas_border.shape[1], pen_border[0] + border_bitmap_width)

            # 检查切片是否有效（宽度和高度都大于0）
            if paste_border_y_start >= paste_border_y_end or paste_border_x_start >= paste_border_x_end:
                logger.warning(
                    f"The border of char '{cdpt}' is completely outside the canvas or on the boundary, skipped. Position: x={pen_border[0]}, y={pen_border[1]}, Canvas size: {canvas_border.shape}")
            else:
                # 确保切片源和目标尺寸匹配
                # Ensure slice source and target dimensions match
                ...
                if bitmap_border_slice.size > 0:
                    ...
                    if target_slice.shape == bitmap_border_slice.shape:
                        ...
                        logger.warning(
                            f"Shape mismatch: target={target_slice.shape}, source={bitmap_border_slice.shape}")
...
def put_text_vertical(font_size: int, text: str, h: int, alignment: str, fg: Tuple[int, int, int], bg: Optional[Tuple[int, int, int]], line_spacing: int):
    ...
    canvas_x = font_size * num_char_x + spacing_x * \
        (num_char_x - 1) + (font_size + bg_size) * 2
    ...
    # pen (x, y)
    pen_orig = [canvas_text.shape[1] -
                (font_size + bg_size), font_size + bg_size]
...
def select_hyphenator(lang: str):
    ...
    installed_langs = HYPHENATOR_LANGUAGES()
    if lang not in installed_langs:
        for avail_lang in reversed(installed_langs):
...
def get_string_width(font_size: int, text: str):
    if not text:
        return 0
    width = get_char_offset_x(font_size, text[0])
    for i in range(1, len(text)):
        kern = get_pair_kerning(text[i-1], text[i], font_size, 0)
        width += kern + get_char_offset_x(font_size, text[i])
    return width
...
def calc_horizontal(font_size: int, text: str, max_width: int, max_height: int, language: str = 'en_US', hyphenate: bool = True) -> Tuple[List[str], List[int]]:
    ...
    while True:
        ...
        expected_size = sum(word_widths) + max((len(word_widths) - 1)
                                               * whitespace_offset_x - (max_lines - 1) * hyphen_offset_x, 0)
    ...
    for i, word in enumerate(words):
        ...
        if hyphenator and len(word) <= 100:
            try:
                new_syls = hyphenator.wrap(word, width=100)[0].split('-')
            except Exception:
                new_syls = []
        if len(new_syls) == 0:
            if len(word) <= 3:
    ...
    def get_present_syllables(line_idx, word_pos):
        syl_start_idx, syl_end_idx = get_present_syllables_range(
            line_idx, word_pos)
        ...
        while line_idx < len(line_words_list) - 1:
            ...
            while len(line_words2) != 0:
                ...
                if first_word and word_idx == line_words1[-1]:
                    ...
                    syl_start_idx = 0
                    syl_end_idx = len(syllables[word_idx]) if len(
                        line_words2) > 1 else hyphenation_idx_list[line_idx + 1]
                first_word = False
    ...
    while line_idx < len(line_words_list) - 1:
        ...
        if line_words1[-1] == line_words2[0]:
            ...
            if len(word2_text) == 1 or word2_width < font_size:
                ...
                line_width_list[line_idx + 1] -= word2_width + \
                    whitespace_offset_x
    ...
    use_hyphen_chars = hyphenate and hyphenator and max_width > 1.5 * \
        font_size and len(words) > 1
...
def put_char_horizontal(font_size: int, cdpt: str, pen_l: Tuple[int, int], canvas_text: np.ndarray, canvas_border: np.ndarray, border_size: int):
    ...
    if border_size > 0:
        ...
        if (border_bitmap_rows * border_bitmap_width > 0 and
            ...
            paste_border_y_end = min(
                canvas_border.shape[0], pen_border_y + border_bitmap_rows)
            paste_border_x_end = min(
                canvas_border.shape[1], pen_border_x + border_bitmap_width)
            ...
            border_slice_y_end = border_slice_y_start + \
                (paste_border_y_end - paste_border_y_start)
            border_slice_x_end = border_slice_x_start + \
...
def put_text_horizontal(font_size: int, text: str, width: int, height: int, alignment: str,
    ...
    # calc
    # print(width)
    line_text_list, line_width_list = calc_horizontal(
        font_size, text, width, height, lang, hyphenate)
    # print(line_text_list, line_width_list)

    # make large canvas
    canvas_w = max(line_width_list) + (font_size + bg_size) * 2
    canvas_h = font_size * len(line_width_list) + spacing_y * \
        (len(line_width_list) - 1) + (font_size + bg_size) * 2
    ...
    for line_text, line_width in zip(line_text_list, line_width_list):
        ...
        if alignment == 'center':
            pen_line[0] += (max(line_width_list) - line_width) // 2 * \
                (-1 if reversed_direction else 1)
...
def test():
    ...
    canvas = put_text_horizontal(
        64, 1.0, '因为不同‼ [这"真的是普]通的》肉！那个“姑娘”的恶作剧！是吗？咲夜⁉', 400, (0, 0, 0), (255, 128, 128))
