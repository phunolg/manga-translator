...
def dump_image(img_pil: Image.Image, img: np.ndarray, alpha_ch: Image.Image = None):
    ...
    print(f"dump_image: img_pil type = {type(img_pil)}, img type = {type(img)}, alpha_ch type = {type(alpha_ch)}")
...
class Quadrilateral(object):
    ...
    @staticmethod
    def _pad_quad_bbox(pts: np.ndarray, pixels: float) -> np.ndarray:
        x_min = np.min(pts[:, 0]) - pixels
        x_max = np.max(pts[:, 0]) + pixels
        y_min = np.min(pts[:, 1]) - pixels
        y_max = np.max(pts[:, 1]) + pixels
    ...
    def get_transformed_region(self, img, direction, textheight, expand_px: int = 1) -> np.ndarray:
        [l1a, l1b, l2a, l2b] = [a.astype(np.float32) for a in self.structure]
        v_vec = l1b - l1a
        h_vec = l2b - l2a
        ratio = np.linalg.norm(v_vec) / np.linalg.norm(h_vec)

        src_pts = self.pts.astype(np.float32).copy()
        src_pts = self._pad_quad_bbox(src_pts, expand_px)
...
def quadrilateral_can_merge_region(a: Quadrilateral, b: Quadrilateral, ratio = 1.9, discard_connection_gap = 0.75, char_gap_tolerance = 0.6, char_gap_tolerance2 = 1.5, font_size_ratio_tol = 1.5, aspect_ratio_tol = 2) -> bool:
    b1 = a.aabb
    b2 = b.aabb
    char_size = min(a.font_size, b.font_size)
    # print(f"Font size {char_size}")
    x1, y1, w1, h1 = b1.x, b1.y, b1.w, b1.h
    x2, y2, w2, h2 = b2.x, b2.y, b2.w, b2.h
    print(f"Box 1: {x1, y1, w1, h1}, Box 2: {x2, y2, w2, h2}")
    ...
    print(f"Distance between boxes: {dist}, discard_connection_gap * char_size: {discard_connection_gap * char_size}, a_aa: {a_aa}, b_aa: {b_aa}")
    if a_aa and b_aa:
