...
def get_upscaler(key: Upscaler, *args, **kwargs) -> CommonUpscaler:
    if key not in UPSCALERS:
        raise ValueError(
            f'Could not find upscaler for: "{key}". Choose from the following: %s' % ','.join(UPSCALERS))
...
async def unload(upscaler_key: Upscaler):
    upscaler = upscaler_cache.pop(upscaler_key, None)
    if upscaler and isinstance(upscaler, OfflineUpscaler):
    ...
    try:
        ...
        logger.warning(f"Error during upscaling unload", exc_info=True)
        pass
