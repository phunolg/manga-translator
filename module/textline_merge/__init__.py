...
def merge_bboxes_text_region(bboxes: List[Quadrilateral], width, height):
    ...
    for ((u, ubox), (v, vbox)) in itertools.combinations(enumerate(bboxes), 2):
        ...
        if quadrilateral_can_merge_region(ubox, vbox,
                                        aspect_ratio_tol=1.5,
                                        font_size_ratio_tol=2,
                                        char_gap_tolerance=2,
                                        char_gap_tolerance2=5):
            G.add_edge(u, v)
        print(f"u {u} box pts: {ubox.pts}, v {v} box pts: {vbox.pts}, can_merge: {G.has_edge(u, v)}")
    ...
    region_indices: List[Set[int]] = list(nx.algorithms.components.connected_components(G))
        
    logger.debug("Edges in G:")
    for node in region_indices:
        logger.debug(f"Region with nodes: {node}")
    ...
    # for node_set in nx.algorithms.components.connected_components(G):
        ...
        print("Merging textlines into text region:")
        for i, box in enumerate(txtlns):
            print(f"Box {i}: pts={box.pts}, text='{box.text}'")
